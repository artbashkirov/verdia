...
- Если в контексте недостаточно данных для конкретики — верни ровно одну пустую строку.`;
    ...
    if (shouldGenerateDocuments) {
      ...
      // ── Hard-ban для возражений по налоговым искам ───────────────────────
      // Несмотря на правила в DOCUMENT_GENERATION_PROMPT и в шаблоне
      // tax-claim-individual, Gemini продолжает писать «пропуск срока на 2 дня»
      // главным основанием — оно теряется в большом контексте. Этот блок
      // идёт ПОСЛЕДНИМ перед заданием, чтобы инструкция была «свежей».
      //
      // Условия применения:
      //   • в triage documentType явно налоговый, ИЛИ
      //   • в тексте задания/контекста явные налоговые маркеры
      //     (ИФНС, НК РФ, страховые взносы и т.п.)
      const taxIndicators =
        /(?:ифнс|нк\s?рф|налог[оа]\w*|страховы\w*\s+взнос|административн\w*\s+иск\w*\s+о\s+взыскани\w*\s+(?:задолженност\w*|недоимк\w*|пен[ие]\w*))/i;
      const isTaxObjection =
        isObjectionLikeDocument(messageForAi, isTriageChat ? gen.response?.documentType : undefined) &&
        (taxIndicators.test(messageForAi) || taxIndicators.test(contextSummary));

      if (isTaxObjection) {
        messages.push({
          role: 'user',
          content: `=== ВНИМАНИЕ. КРИТИЧЕСКИ ВАЖНОЕ ПРАВИЛО ДЛЯ ЭТОГО КОНКРЕТНОГО ДОКУМЕНТА ===

Это возражение на АДМИНИСТРАТИВНЫЙ ИСК НАЛОГОВОГО ОРГАНА (ИФНС) о взыскании налогов / страховых взносов / пеней с физического лица в порядке ст. 48 НК РФ.

ЗАПРЕЩЕНО (под угрозой полной непригодности документа):

1. ❌ Использовать «пропуск срока на подачу административного иска» как основание возражения. НЕЛЬЗЯ. Совсем.
   — Налоговый орган соблюдает срок при ПЕРВОМ обращении в суд (заявление о судебном приказе). Подача иска после отмены приказа — продолжение начатой процедуры; пропуск 6-месячного срока на повторное обращение НЕ является пресекательным и восстанавливается судом.
   — Дата на самом иске (исходящий номер ИФНС) — это дата составления, а не дата подачи. Без почтового штемпеля / отметки канцелярии / квитанции ГАС «Правосудие» утверждать о пропуске невозможно.
   — Если в твоём тексте появляется фраза «пропуск срока», «срок пропущен», «истец опоздал», «6 месяцев истекли», «обратился за пределами срока» — УДАЛИ это основание целиком. Не оставляй ни в одном виде, даже как дополнительный довод.

2. ❌ Не оспаривать факт получения требования об уплате, если в материалах есть номер требования с датой — ответчик его получал.

3. ❌ Не использовать сослагательные формулировки («возможно», «вероятно», «может быть») в основаниях.

ЧТО ДЕЛАТЬ ВМЕСТО ЭТОГО (главные направления возражения по такому делу):

A. КОНТР-РАСЧЁТ ПЕНИ с учётом моратория, установленного Постановлением Правительства РФ от 28.03.2022 № 497 (период 01.04.2022 — 01.10.2022). Структура обязательна:
   • выписать заявленные истцом пени по периодам (за 2022, 2023, 2024);
   • выделить часть пени за 2022 год, относящуюся к периоду моратория, и обосновать её исключение;
   • привести итоговую справедливую сумму пени по контр-расчёту ответчика;
   • указать разницу в пользу ответчика.

B. ХОДАТАЙСТВО О СНИЖЕНИИ ПЕНИ ПО СТ. 333 ГК РФ (если соотношение пени к основному долгу заметное — обычно более 30%). Обосновать несоразмерность последствиям нарушения.

C. ХОДАТАЙСТВО ОБ ИСТРЕБОВАНИИ ДЕТАЛЬНОГО РАСЧЁТА у истца (ст. 63 КАС РФ), если в приложениях к иску нет разбивки пени по периодам и ставкам.

D. (Если применимо) Имущественное положение ответчика — ходатайство о рассрочке (ст. 358 КАС РФ).

Если RAG не вернул ст. 333 ГК РФ или Постановление № 497 в блоке «БАЗА НПА» — сошлись на них в общей форме («Постановление Правительства РФ № 497 от 28.03.2022») и пометь как «требует проверки юристом конкретная норма», но ОБЯЗАТЕЛЬНО включи направления A и B в текст возражения. Это сильные доводы.

Главный аргумент защиты по такому делу — расчёт пени и её снижение, а НЕ срок.`,
        });
        messages.push({
          role: 'assistant',
          content:
            'Понял. Срок не оспариваю. Главное — контр-расчёт пени с учётом моратория № 497, ст. 333 ГК РФ для снижения, и ходатайства об истребовании детального расчёта.',
        });
      }
