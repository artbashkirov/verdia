  ...
  const drainUploadQueue = async () => {
    ...
    try {
      while (uploadQueueRef.current.length > 0) {
        ...
        // ВАЖНО: загрузка строго последовательная. Параллельные запросы
        // к Replicate (GLM-OCR) ловят несколько одновременных cold start
        // и/или упираются в лимит конкурентных предсказаний — в итоге
        // всё висит и падает по таймауту. Последовательно: первый файл
        // ловит cold start (~60-90 с), последующие переиспользуют тот
        // же прогретый GPU-инстанс (~10-15 с). Итого 3 файла ≈ 90-120 с
        // вместо 3-5 минут при параллельной обработке.
