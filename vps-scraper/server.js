...
function detectResult(text) {
  ...
  for (const p of partialPatterns) if (lastPart.includes(p)) return 'частично удовлетворен';
  ...
  for (const p of rejectedPatterns) if (lastPart.includes(p)) return 'отказано';
  ...
  for (const p of satisfiedPatterns) {
    if (lastPart.includes(p)) {
      ...
      const context = lastPart.slice(Math.max(0, idx - 50), idx + p.length + 50);
